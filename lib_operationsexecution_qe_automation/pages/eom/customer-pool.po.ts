...
export class CustomerPoolPage extends BasePage {
    ...
    selectDivisionType: ElementFinder = element(by.xpath("//select[@id='form:division']/option"));
    customerCode: ElementFinder = element(by.xpath("//input[@id='form:customerCode']"));
    ...
    readonly searchButton: ElementFinder = element(by.xpath("//button[@id='form:searchButton']"))
    ...
    public async getEquipmentNumberFromCustomerPoolTable(columnHeader1: string, columnValue1: string, columnHeader2: string, columnValue2: string, requiredColumn: string) {
        ...
        do {
            ...
            for (let i = 1; i <= rowCount; i++) {
                ...
                console.log("prefix" + await ele_Prefix.getText());
                console.log("prefix" + await ele_LTSt.getText());
                if (((await ele_Prefix.getText()).trim() === columnValue1) && ((await ele_LTSt.getText()).trim() === columnValue2)) {
                    ...
                    return (await ele_Equipment.getText()).trim();
                }
            }
    ...
    public async getEquipmentNumber() {
        ...
        let equipmentNumberXpath = element(by.xpath("//tr[contains(@id,'form:equipmentPoolTable:')]/td[2]//span[@class='iceOutTxt']/ancestor::tr/td[8]//span[@class='iceOutTxt']/ancestor::tr/td[3]//span[@class='iceOutTxt']"));
        let equipmentCodeXpath = element(by.xpath("//tr[contains(@id,'form:equipmentPoolTable:')]/td[2]//span[@class='iceOutTxt']/ancestor::tr/td[8]//span[@class='iceOutTxt']/ancestor::tr/td[3]//span[@class='iceOutTxt']/ancestor::tr/td[2]//span[@class='iceOutTxt']"));
        try {
            do {
                equipment[0] = await equipmentNumberXpath.getText();
                if (equipment[0] !== null) {
                    equipment[1] = await equipmentCodeXpath.getText();
                    break;
                }
            } while (await this.pagination());
        } catch (error) {
            console.log("equipment number error " + error)
        }
