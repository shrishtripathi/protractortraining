...
//TC 131769: EOM_Regressions_27 Adding stops and deleting stops 
using(DataProvider.Tc_131769, async function (data) {
    describe("making changes to an order", () => {
        ...
        //TC_131748 EOM_Regression_25 Adding comments to a order
        ...
        //TC_131777 EOM_Regressions_30 Changing and deleting comments in a order
        ...
        //131769 EOM_Regressions_27 Adding stops and deleting stops 
        it("Should select Shipper from the drop down", async () => {
            await detailsPage.selectDepartmentDropDown(data.shipperDropDown)
        });

        it("Should enter the Shipper Code", async () => {
            await detailsPage.enterShipperCode(data.shipperCode);
        });

        it("Set current date, shipper scheduled appointment time to 23:30-23:59", async () => {
            await detailsPage.setCurrentDate();
            await detailsPage.setScheduledAppointmentTime(data.shipperBeginTime, data.shipperEndTime)
        });

        it("Should select the customer drop down from load/ unload by ", async () => {
            await detailsPage.selectDropDownsFromDetailsPage(data.customerDropDown)
        })

        it("Should select 'N' from Driver Count drop down ", async () => {
            await detailsPage.selectDriverCount(data.driverCount)
        })
        ...
        it("Should click on Save Changes Button", async () => {
            await detailsPage.clickOnSaveChangesButton(data.saveChangesButton);
        });

        it("Should click on Expand Arrow Button", async () => {
            await detailsPage.clickOnExpandArrow();
        });

        it("Should click on Delete Button", async () => {
            await detailsPage.clickOnDeleteButton();
        });
        ...
        it("Should click on Cancel Load Button", async () => {
            await detailsPage.clickOnSaveChangesButton(data.cancelLoadButton);
        });

        it("Should select the customer drop down from load/ unload by ", async () => {
            await detailsPage.selectDropDownsFromDetailsPage(data.lateNotice)
        })

        it("Should click on Confirm Button", async () => {
            await detailsPage.clickConfirmButton();
        });
