...
//Tc #125144 - IPAY_Regression_13 Over Hours Report
using(DataProvider.Tc_125144, async function (data) {

    describe("IPAY_Regression_13 Over Hours Report", function () {
        ...
        it("Should click on Over Hours Report under the Workflow Menu", async () => {
            await iPayHomePage.clickOnLinkMenu(data.overHoursReportLink);
        });
        ...
        it("Verifying that Board Code will appear", async () => {
            await expect(activityWorkFlowPage.verifyReport(data.board)).toBeTruthy();
        });
