...
//TC #132694: Maximo_Verify Item Type is not NOCHARGE or MISC
using(DataProvider.Tc_132694, async function (data) {
    let prNumber = null;
    let prNumber1 = null;

    describe("Verify Item Type is not NOCHARGE or MISC", () => {
        ...
        it("Click on Save icon", async () => {
            // await inventoryPage.clickOnInputButton(inventoryPage.saveButton);
            await inventoryPage.click(inventoryPage.saveButton);
        });
        ...
        it("Click the Save Icon at the top of the screen. ", async () => {
            await inventoryPage.clickOnSaveButton();
        });
        it("Verify save message at the top", async () => {
            let result: boolean = await inventoryPage.verifySavedMessage();
            await expect(result).toBeTruthy();
        ...
        //# TC 132694
        it("Select the three line icon at the top of the screen", async () => {
            await maximoHomePage.clickMenuIcon();
        });
        it("Hover over Inventory", async () => {
            await maximoHomePage.mouseHoverInventory();
        });
        it("Select Item Master", async () => {
            await maximoHomePage.clickItemMaster();
        });
        it(" Enter the 'item number' in the field 'find item'", async () => {
            await itemMasterPage.enterInFindItem(itemMasterPage.itemValue)
        ...
        it("Click the hyperlink for your item.", async () => {
            await itemMasterPage.clickOnLink(itemMasterPage.itemValue);
        });
        it("Verify that the Item Type IS NOT NOCHARGE or MISC.", async () => {
            await itemMasterPage.hitEnter();
        });
