...
//TC 131551 :EOM_Regression_23 Check calls
using(DataProvider.Tc_131551, async function (data) {
  describe("EOM_Regression_23 Check calls", function () {
    ...
    using(DataProvider.Tc_131544, async function (data1) {
      ...
      it("set values in customer pool page", async () => {
        await customerPoolPage.setValuesInEquipmentPoolLookup(data1.customerPool_Division, data1.customerPool_CustCode, data1.customerPool_EquipementCLass, "");
      });
    ...
    using(DataProvider.Tc_131546, async function (data1) {
      it("Hover status Option from header", async () => {
        await freightManagerPage.hoverOverFM2PageHeaderOption(data1.headerOptionStatus);
      });
      1
      it("Click on dispatch option", async () => {
        await freightManagerPage.clickHeaderDropDownOption(data1.dropDownOptionDispatch);
      });
      ...
      it("Verify dispatch row under check call history", async () => {
        let result: boolean = await checkCallDetailsPage.verifyCheckCallHistorySuccessful(data1.checkCallHistoryText);
