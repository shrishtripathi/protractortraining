...
// TC #71602 :Validating the Reject Tender and Cancel Buttons while rejecting a tender.
using(DataProvider.Tc_71602, async function (data) {

    describe('Validating the Reject Tender and Cancel Buttons while rejecting a tender', () => {
        let loadNumber;
        let projectCode;
        ...
        //    await loadDetailsPage.changeProjectCode(data.projectCodeValue)
 ...
 */
        ...
        it('Click on the Reject Tender Button in the Tender Agreement Page', async() => {
            await acceptanceNeededPage.clickOnElement(acceptanceNeededPage.rejectTenderButton);
        });

        it('Verify reject tender popup message', async() => {
            let result:boolean = await acceptanceNeededPage.verifyRejectTenderPopup();
            await expect(result).toBeTruthy();
        });

        it('verify color of Reject tender Button when normal', async() => {
           let color:any = await acceptanceNeededPage.checkBackgroundColorOfElement(acceptanceNeededPage.rejectTenderButtonOnPopup);
           await expect(color).toBe(data.defaultRejectTenderButtonColor);
           let fontColor:any = await acceptanceNeededPage.checkFontColorOfElement(acceptanceNeededPage.rejectTenderSpanTextPopup);
           await expect(fontColor).toBe(data.buttonTextFontColor);
           let borderColor:any = await acceptanceNeededPage.checkBorderColorOfElement(acceptanceNeededPage.rejectTenderButtonOnPopup)
           await expect(borderColor).toContain(data.defaultRejectTenderButtonBorderColor);
        });

        it('verify color of Reject tender button when its hovered', async() => {
            await acceptanceNeededPage.actions.moveMouseToElement(acceptanceNeededPage.rejectTenderButtonOnPopup,"");
            let color = await acceptanceNeededPage.checkBackgroundColorOfElement(acceptanceNeededPage.rejectTenderButtonOnPopup);
        ...
        it('verify color of Cancel button when its hovered', async() => {
            ...
            let fontColor:any = await acceptanceNeededPage.checkFontColorOfElement(acceptanceNeededPage.cancelButtonSpanTextPopup);
            await expect(fontColor).toBe(data.cancelButtonFontColorHover);
            let borderColor:any = await acceptanceNeededPage.checkBorderColorOfElement(acceptanceNeededPage.cancelButtonOnPopup)
