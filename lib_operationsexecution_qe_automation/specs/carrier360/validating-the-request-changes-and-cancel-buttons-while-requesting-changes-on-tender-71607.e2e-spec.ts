...
// TC #71607 :C360 |Validating the Request Changes and Cancel Buttons while Requesting Changes on a tender.
using(DataProvider.Tc_71607, async function (data) {

    describe('Validating the Request Changes and Cancel Buttons while Requesting Changes on a tender', () => {
        let loadNumber;
        let projectCode;
        ...
        it('Click on the Request Changes Button in the Tender Agreement Page', async () => {
            await acceptanceNeededPage.clickOnElement(acceptanceNeededPage.requestChangesButton);
        });

        it('Verify Request changes window pop up', async () => {
            let result:boolean = await acceptanceNeededPage.verifyRequestChangesPopup();
            await expect(result).toBeTruthy();
        });

        it('verify color of Request changes Button when normal', async () => {
            let color: any = await acceptanceNeededPage.checkBackgroundColorOfElement(acceptanceNeededPage.requestChangesButtonOnPopup);
            await expect(color).toBe(data.defaultRequestChangesButtonColor);
            let fontColor:any = await acceptanceNeededPage.checkFontColorOfElement(acceptanceNeededPage.requestChangesSpanTextPopup)
            await expect(fontColor).toBe(data.buttonTextFontColor);
            let borderColor:any = await acceptanceNeededPage.checkBorderColorOfElement(acceptanceNeededPage.requestChangesButtonOnPopup)
            await expect(borderColor).toContain(data.defaultRequestChangesButtonBorderColor);
        });

        it('verify color of Request changes button when its hovered', async () => {
            await acceptanceNeededPage.actions.moveMouseToElement(acceptanceNeededPage.requestChangesButtonOnPopup, "");
            let color = await acceptanceNeededPage.checkBackgroundColorOfElement(acceptanceNeededPage.requestChangesButtonOnPopup);
            await expect(color).toBe(data.mouseHoverRequestChangesButtonColor);
            let fontColor:any = await acceptanceNeededPage.checkFontColorOfElement(acceptanceNeededPage.requestChangesSpanTextPopup)
            await expect(fontColor).toBe(data.buttonTextFontColor);
            let borderColor:any = await acceptanceNeededPage.checkBorderColorOfElement(acceptanceNeededPage.requestChangesButtonOnPopup)
            await expect(borderColor).toContain(data.mouseHoverRequestChangesButtonBorderColor);
        });

        it('verify color of Cancel button when normal', async () => {
            let color: any = await acceptanceNeededPage.checkBackgroundColorOfElement(acceptanceNeededPage.cancelButtonOnPopup);
            await expect(color).toBe(data.defaultCancelButtonColor);
            let fontColor:any = await acceptanceNeededPage.checkFontColorOfElement(acceptanceNeededPage.cancelButtonSpanTextPopup)
            await expect(fontColor).toBe(data.cancelButtonFontColor);
            let borderColor:any = await acceptanceNeededPage.checkBorderColorOfElement(acceptanceNeededPage.cancelButtonOnPopup)
            await expect(borderColor).toContain(data.defaultCancelButtonBorderColor);
        });

        it('verify color of Cancel button when its hovered', async () => {
            await acceptanceNeededPage.actions.moveMouseToElement(acceptanceNeededPage.cancelButtonOnPopup, "");
            let color: any = await acceptanceNeededPage.checkBackgroundColorOfElement(acceptanceNeededPage.cancelButtonOnPopup);
            await expect(color).toBe(data.mouseHoverCancelButtonColor);
            let fontColor:any = await acceptanceNeededPage.checkFontColorOfElement(acceptanceNeededPage.cancelButtonSpanTextPopup)
            await expect(fontColor).toBe(data.mouseHoverCancelButtonFontColor);
            let borderColor:any = await acceptanceNeededPage.checkBorderColorOfElement(acceptanceNeededPage.cancelButtonOnPopup)
            await expect(borderColor).toContain(data.mouseHoverCancelButtonBorderColor);
        });
