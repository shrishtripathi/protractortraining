...
// TC #71597 :C360 | Validating the Accept Tender and Cancel Buttons while accepting a tender.
using(DataProvider.Tc_71597, async function (data) {

    describe('Validating the Accept Tender and Cancel Buttons while accepting a tender ', () => {
        let loadNumber;
        let projectCode;
...
/*         it('IE-Turn off Windows Integrated Authority', async () => {
        ...
        it('Click on the Accept Tender Button in the Tender Agreement Page', async () => {
            await acceptanceNeededPage.clickOnElement(acceptanceNeededPage.acceptTenderButton);
        });

        it('Verify Accept Tender confirmation pop up', async () => {
            let result:boolean = await acceptanceNeededPage.verifyAcceptTenderPopup();
            await expect(result).toBeTruthy();
        });

        it('verify color of Accept tender Button when normal', async () => {
            let color: any = await acceptanceNeededPage.checkBackgroundColorOfElement(acceptanceNeededPage.acceptTenderButtonOnPopup);
            await expect(color).toBe(data.defaultAcceptTenderButtonColor);
            let fontColor:any = await acceptanceNeededPage.checkFontColorOfElement(acceptanceNeededPage.acceptTenderSpanTextPopup)
            await expect(fontColor).toBe(data.buttonTextFontColor);
            let borderColor:any = await acceptanceNeededPage.checkBorderColorOfElement(acceptanceNeededPage.acceptTenderButtonOnPopup)
            await expect(borderColor).toContain(data.defaultAcceptTenderButtonBorderColor);
        });

        it('verify color of Accept tender button when its hovered', async () => {
            await acceptanceNeededPage.actions.moveMouseToElement(acceptanceNeededPage.acceptTenderButtonOnPopup, "");
            let color = await acceptanceNeededPage.checkBackgroundColorOfElement(acceptanceNeededPage.acceptTenderButtonOnPopup);
