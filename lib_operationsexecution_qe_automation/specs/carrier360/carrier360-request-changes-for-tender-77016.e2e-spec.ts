...
// TC #77016 : Carrier 360 Request changes for tender
using(DataProvider.Tc_77016, async function (data) {

    describe('Request changes for tender', () => {
        let loadNumber;
        let projectCode;
        ...
        it('verify text should be greater than 10 characters', async () => {
            await acceptanceNeededPage.enterTextInTenderBox(data.textLessThanTen);
            await acceptanceNeededPage.clickOnRequestChangesButtonOnPopup();
            await expect(<boolean>await acceptanceNeededPage.verifyWarningMessage(data.lessThanTenWarning)).toBeTruthy();
        });
