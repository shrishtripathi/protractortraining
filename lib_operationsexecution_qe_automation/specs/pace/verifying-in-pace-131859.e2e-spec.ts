...
//TC 131859: PACE_Regression_6 Part 3  Verifying in pace
using(DataProvider.Tc_131859, async function (data) {
    let orderNumber: string;
    describe("PACE_Regression_6 Part 3 Verifying in pace", () => {
        ...
        using(DataProvider.Tc_131858, async function (data) {
            //let orderNumber: string = this.loadnumber1;
            let equipmentNumber: string;
            let tractorNumber: string;
            let projectCode: string = "S001";
        ...
        it("Should set Whirlpool LDCN - HOUTXX to account dropdown", async () => {
        ...
        it("Enter JBH Order # value", async () => {
            await orderLookupPage.selectRadioButton(data.jbhOrder);
            await orderLookupPage.jbhOrderText(data.jbhOrder, orderNumber);
        });
        ...
        it("Click on Order Number", async () => {
            await editTripPage.clickOnOrderNumberLink();
        });

        it("Switch to another tab", async () => {
            await editTripPage.actions.selectWindow(1, "navigate to order details");
        });

        it("Verify division  and fleet code", async () => {
            let result: Boolean = await editTripPage.verifyDivisionFleetCode(data.division, data.fleetCode);
            expect(result).toBeTruthy();
        });
    });
