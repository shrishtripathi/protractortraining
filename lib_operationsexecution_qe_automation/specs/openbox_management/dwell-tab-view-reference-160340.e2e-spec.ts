...
//Tc# 160340 - Box Management System App- DWELL TAB- 'VIEW REFERENCE #' (via OUTGATED Trailer Summary Screen)
using(DataProvider.Tc_160340, async function (data) {

    describe("DWELL TAB- View reference", function () {
        let loadNumber;
        ...
        it("Should click on view reference # button", async () => {
            await dwellPage.click(dwellPage.viewReferenceButton);
            await dwellPage.waitForActionToComplete();
        });
                
        it("Verifying reference page is displayed", async () => {
