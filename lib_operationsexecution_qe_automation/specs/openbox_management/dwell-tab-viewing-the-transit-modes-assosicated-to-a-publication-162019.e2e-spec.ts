...
//Tc# 162019 - Box Management System App- DWELL TAB- Viewing the 'TRANSIT MODES' Assosicated to a Publication  (via the Transit Mode filter on theOUTGATED Trailer Summary Screen)
using(DataProvider.Tc_162019, async function (data) {

    describe("Box Management System App- DWELL TAB- Viewing the 'TRANSIT MODES' Assosicated to a Publication  (via the Transit Mode filter on theOUTGATED Trailer Summary Screen)", function () {
        ...
        it("Should click on Transit Mode dropdown and verify the list will appear displaying one or more 'DIVISION' Locations", async () => {
            await dwellPage.click(dwellPage.transitModeDropdown);
            expect(await dwellPage.transitModeDropdownOptions.count()).toBeGreaterThan(data.one); 
        });
