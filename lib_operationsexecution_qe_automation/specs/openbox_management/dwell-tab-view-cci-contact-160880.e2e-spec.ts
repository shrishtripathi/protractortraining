...
//Tc# 160880 - Box Management System App- DWELL TAB- VIEW CCI CONTACTS (via OUTGATED Trailer Summary Screen)
using(DataProvider.Tc_160880, async function (data) {

    describe("DWELL TAB- View cci contacts", function () {
        let loadNumber;
        ...
        it("Should click on publication", async () => {
            await dwellPage.click(trailerPage.publicationSecondRow);
            await dwellPage.waitForActionToComplete();
        });
        ...
        it("Should click on view cci contacts button", async () => {
            await dwellPage.click(dwellPage.viewCCIContactsButton);
            await dwellPage.waitForActionToComplete();
        });
                
        it("Verifying cci contacts page is displayed", async () => {
