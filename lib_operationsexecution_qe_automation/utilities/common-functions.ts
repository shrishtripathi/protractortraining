...
export class CommonFunctions extends BasePage {
	...
	/* 
		async mssqlqueryHandler(conn_String: any, sql_Query: string, return_Data: boolean) {
			try {
				let pool = new sql.Connection(conn_String);
				await pool.connect();
		...
		switch (DBName.toLowerCase()) {
			...
			case 'mssql':
				try {
					...
					let conn = new sql.Connection(conn_String);
					await conn.connect();
	...
	public async getUnitViewTableIndex(tableid: string, columnName: string) {
		let tblIndex: number = 0;
		let flag: boolean = true;
		try {
			let tableHeadersAll: ElementArrayFinder = element.all(By.xpath("//*[@id='form:" + tableid + "']//th"));
			let tableHeadersCount = await tableHeadersAll.count();
			console.log("total rows : "+tableHeadersCount);
			for (let i: number = 1; i <= tableHeadersCount / 2; i++) {
				let tableHeader1: ElementFinder = element(By.xpath("(//*[@id='form:" + tableid + "']//th[" + i + "])[1]"));
				let tableHeader2: ElementFinder = element(By.xpath("(//*[@id='form:" + tableid + "']//th[" + i + "])[2]"));
				let tableHeaderText1:string = await tableHeader1.getText();
				let tableHeaderText2:string = await tableHeader2.getText();
			//	await console.log("Column Text1: " + tableHeaderText1)
			//	await console.log("Column Text2: " + tableHeaderText2)
				if (tableHeaderText1 + "\n" + tableHeaderText2 === columnName) {
					await console.log("index" + i);
					tblIndex = i;
					break;
				}
