...
let loadCreateData = Object.assign({}, DataProvider.Tc_187738);
let eomE2EComponents = new EOME2EComponents();
let fm2E2EComponents = new FM2E2EComponents();

//TC 187738: FM2 Dispatch Verification
using(loadCreateData, async (data) => {
    
    describe('FM2 Dispatch Verification', async () => {
        ...
        describe("FM2 Dispatch Verification",async () => {
            await fm2E2EComponents.dispatchVerification(data.onePickOrDropTruckModeLoadData, data.fm2dispatchVerificationData)
        });
