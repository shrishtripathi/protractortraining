...
//TC 188223: EOM ICS Temp Control Load Create
using(DataProvider.Tc_188212, async function (data) {
    describe("EOM ICS Temp Control Load Create Verification", () => {
        ...
        it("Verifying There Are No Classifications Selected", async () => {
            await orderDetailsViewPage.selectClassification("REEFER");
        });
        ...
        it("Enter Load number", async function () {
            await enterpriseOrderManagementPage.enterLoadNumber(loadNumber)
        }); 

        it("Should Click On Search Loads Button", async () => {
            await enterpriseOrderManagementPage.clickOnEOMButton("eomSearchMain:advOrderSearch")
        }); 

        it("Should Click On Load number lnk", async () => {
        ...
        it("Under Classifications, ensure it reads REEFER", async () => {
           expect(await orderDetailsViewPage.getClassificationType()).toBe("REEFER");
        });
