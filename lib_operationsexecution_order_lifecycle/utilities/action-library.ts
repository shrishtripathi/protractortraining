...
import { element, by, ExpectedConditions, browser, protractor, ElementFinder, ElementArrayFinder } 
from "protractor";
...
export class ActionLibrary {
        ...
        async waitUntilElementPresenceOf(element: ElementFinder, logname?: string) {
                return await browser.wait(EC.presenceOf(element), 180000);
        }
        ...
        async waitUntilElementVisible(element: ElementFinder, logname: string, waitTime=30000) {
        ...
        async SendKeysDown(logName: string) {
                var tab = await browser.actions().sendKeys(protractor.Key.ARROW_DOWN);
                // await tab.perform();
        }
